...
namespace InnovateTraffic
{
    ...
    {
        // Declare JSON Edison Structure
        public class JSONEdison
        {
            public JSONEdison()
            {
                Name = "";
                Parameters = new Junction_Send();
            }

            public class Junction_Send
            {
                public int north_green_time { get; set; }
                public int north_priority { get; set; }
                public int east_green_time { get; set; }
                public int east_priority { get; set; }
                public int south_green_time { get; set; }
                public int south_priority { get; set; }
                public int west_green_time { get; set; }
                public int west_priority { get; set; }
            }

            public string Name { get; set; }
            //public Dictionary<string, int> Parameters { get; set; }
            public Junction_Send Parameters { get; set; }
        }
        ...
        // Global Parameters
        static int trial_send = 0;
        int total_time, total_emergency_time;
        static int receive_status;
        int trial = 0;
        Junction Junction1;
        Junction Junction2;
        int[] junc1_north_emergency_array = new int[38];
        int[] junc1_east_emergency_array = new int[38];
        int[] junc1_south_emergency_array = new int[38];
        int[] junc1_west_emergency_array = new int[38];
        int[] junc2_north_emergency_array = new int[38];
        int[] junc2_east_emergency_array = new int[38];
        int[] junc2_south_emergency_array = new int[38];
        int[] junc2_west_emergency_array = new int[38];
        ...
        int junc1_remaining_east_emergency_green_time;
        ...
        int[] junc2_remaining_emergency_green_time = new int[4];
        ...
        private async Task ReceiveMessagesFromDeviceAsync(string partition)
        {
            ...
            while (true)
            {
                ...
                string junction_string = Encoding.UTF8.GetString(eventData.GetBytes());
                dynamic junction_data = JsonConvert.DeserializeObject(junction_string);

                if (junction_data.mode == 2)            // Emergency Mode
                {
                    int i_emergency;
                    ...
                    for (i_emergency = 0; i_emergency < total_emergency_time; i_emergency++)
                    {
                        junc1_traffic_light(junc1_north_emergency_array[i_emergency], junc1_east_emergency_array[i_emergency], junc1_south_emergency_array[i_emergency], junc1_west_emergency_array[i_emergency]);
                        junc2_traffic_light(junc2_north_emergency_array[i_emergency], junc2_east_emergency_array[i_emergency], junc2_south_emergency_array[i_emergency], junc2_west_emergency_array[i_emergency]);
                        show_remaining_emergency_time();
                ...
                else if (junction_data.mode == 1)                 // Normal Mode
                {
                    //// Junction 1 Variables Initialization
                    //Junction1.north_traffic = junction_data.nt1;
                    //Junction1.north_green_time = 0;
                    //Junction1.north_priority = 0;
                    //Junction1.east_traffic = junction_data.et1;
                    //Junction1.east_green_time = 0;
                    //Junction1.east_priority = 0;
                    //Junction1.south_traffic = junction_data.st1;
                    //Junction1.south_green_time = 0;
                    //Junction1.south_priority = 0;
                    //Junction1.west_traffic = junction_data.wt1;
                    //Junction1.west_green_time = 0;
                    //Junction1.west_priority = 0;
                    ...
                    //// Junction 2 Variables Initialization
                    //Junction2.north_traffic = junction_data.nt2;
                    //Junction2.north_green_time = 0;
                    //Junction2.north_priority = 0;
                    //Junction2.east_traffic = junction_data.et2;
                    //Junction2.east_green_time = 0;
                    //Junction2.east_priority = 0;
                    //Junction2.south_traffic = junction_data.st2;
                    //Junction2.south_green_time = 0;
                    //Junction2.south_priority = 0;
                    //Junction2.west_traffic = junction_data.wt2;
                    //Junction2.west_green_time = 0;
                    //Junction2.west_priority = 0;
                    ...
                    if (trial == 1)
                    {
                        ...
                        Junction1_SendCloudToDeviceMessageAsync(Junction1);
                        Junction2_SendCloudToDeviceMessageAsync(Junction2);
        ...
        async void Main()
        {
            ...
            for (i = 0; i < 4; i++)
            {
                ...
                // Junction 1 All Direction Array
                junc1_temp_priority = junc1_direction[i];
                //junc1_temp_priority = junc1_priority[i];

                if (junc1_temp_priority == 1)           // North Green
                {
                    for (counter1 = initial1; counter1 < (initial1 + junc1_green_time[(junc1_temp_priority - 1)]); counter1++)
                    {
                        junc1_north_array[counter1] = 3;
                ...
                if (junc1_temp_priority == 2)           // East Green
                {
                    for (counter1 = initial1; counter1 < (initial1 + junc1_green_time[(junc1_temp_priority - 1)]); counter1++)
                    {
                        junc1_east_array[counter1] = 3;
                ...
                if (junc1_temp_priority == 3)           // South Green
                {
                    for (counter1 = initial1; counter1 < (initial1 + junc1_green_time[(junc1_temp_priority - 1)]); counter1++)
                    {
                        junc1_south_array[counter1] = 3;
                ...
                if (junc1_temp_priority == 4)           // West Green
                {
                    for (counter1 = initial1; counter1 < (initial1 + junc1_green_time[(junc1_temp_priority - 1)]); counter1++)
                    {
                        junc1_west_array[counter1] = 3;
            ...
            // Copy data for emergency mode
            total_emergency_time = total_time;

            for (i = 0; i < total_time; i++)
            {
                junc1_north_emergency_array[i] = 1;

                if (i < (total_time - 5))
                {
                    junc1_east_emergency_array[i] = 3;
                }

                else if (i < (total_time - 4))
                {
                    junc1_east_emergency_array[i] = 2;
                }

                else if (i < (total_time - 3))
                {
                    junc1_east_emergency_array[i] = 1;
                }

                junc1_south_emergency_array[i] = 1;
                junc1_west_emergency_array[i] = 1;

                junc2_north_emergency_array[i] = junc2_north_array[i];
                junc2_east_emergency_array[i] = junc2_east_array[i];
                junc2_south_emergency_array[i] = junc2_south_array[i];
                junc2_west_emergency_array[i] = junc2_west_array[i];
            }
        ...
        void Reset()
        {
            ...
            sevenSegment8.Value = "-";
            sevenSegment9.Value = "-";
            sevenSegment10.Value = "-";
            sevenSegment11.Value = "-";
            sevenSegment12.Value = "-";
            sevenSegment13.Value = "-";
            sevenSegment14.Value = "-";
            sevenSegment15.Value = "-";
            sevenSegment16.Value = "-";
        }
        ...
        void calc_total_time()
        {
            ...
            total_time += 18;            // 10 seconds tolerance
        }
        ...
        void calc_remaining_time()
        {
            ...
            junc1_remaining_green_time[junc1_actual_direction[1]] = (junc1_green_time[junc1_actual_direction[0]] + 3);
            junc1_remaining_green_time[junc1_actual_direction[2]] = (junc1_green_time[junc1_actual_direction[0]] + junc1_green_time[junc1_actual_direction[1]] + 4);
            junc1_remaining_green_time[junc1_actual_direction[3]] = (junc1_green_time[junc1_actual_direction[0]] + junc1_green_time[junc1_actual_direction[1]] + junc1_green_time[junc1_actual_direction[2]] + 5);
            ...
            junc2_remaining_green_time[junc2_actual_direction[1]] = (junc2_green_time[junc2_actual_direction[0]] + 3);
            junc2_remaining_green_time[junc2_actual_direction[2]] = (junc2_green_time[junc2_actual_direction[0]] + junc2_green_time[junc2_actual_direction[1]] + 4);
            junc2_remaining_green_time[junc2_actual_direction[3]] = (junc2_green_time[junc2_actual_direction[0]] + junc2_green_time[junc2_actual_direction[1]] + junc2_green_time[junc2_actual_direction[2]] + 5);

            junc2_remaining_emergency_green_time[junc2_actual_direction[0]] = 0;
            junc2_remaining_emergency_green_time[junc2_actual_direction[1]] = (junc2_green_time[junc2_actual_direction[0]] + 3);
            junc2_remaining_emergency_green_time[junc2_actual_direction[2]] = (junc2_green_time[junc2_actual_direction[0]] + junc2_green_time[junc2_actual_direction[1]] + 4);
            junc2_remaining_emergency_green_time[junc2_actual_direction[3]] = (junc2_green_time[junc2_actual_direction[0]] + junc2_green_time[junc2_actual_direction[1]] + junc2_green_time[junc2_actual_direction[2]] + 5);
        }
        ...
        void show_remaining_emergency_time()
        {
            sevensegment_emergency_display();
            ...
            // Improved Code Due To Show Green Time
            if (junc1_remaining_east_emergency_green_time == 0)
            {

            }

            else if (junc1_remaining_east_emergency_green_time > 0)
            {
                junc1_remaining_east_emergency_green_time--;
            }

            if (junc2_remaining_emergency_green_time[0] == 0)
            {

            }

            else if (junc2_remaining_emergency_green_time[0] > 0)
            {
                junc2_remaining_emergency_green_time[0]--;
            }

            if (junc2_remaining_emergency_green_time[1] == 0)
            {

            }

            else if (junc2_remaining_emergency_green_time[1] > 0)
            {
                junc2_remaining_emergency_green_time[1]--;
            }

            if (junc2_remaining_emergency_green_time[2] == 0)
            {

            }

            else if (junc2_remaining_emergency_green_time[2] > 0)
            {
                junc2_remaining_emergency_green_time[2]--;
            }

            if (junc2_remaining_emergency_green_time[3] == 0)
            {

            }

            else if (junc2_remaining_emergency_green_time[3] > 0)
            {
                junc2_remaining_emergency_green_time[3]--;
            }
        ...
        void sevensegment_emergency_display()
        {
            int temp_sevensegment;

            if (junc1_remaining_east_emergency_green_time < 10)
            {
                sevenSegment3.Value = Convert.ToString(0);
                sevenSegment4.Value = Convert.ToString(junc1_remaining_east_emergency_green_time);
            }

            else if (junc1_remaining_east_emergency_green_time >= 10)
            {
                temp_sevensegment = junc1_remaining_east_emergency_green_time / 10;
                sevenSegment3.Value = Convert.ToString(junc1_remaining_east_emergency_green_time / 10);
                sevenSegment4.Value = Convert.ToString(junc1_remaining_east_emergency_green_time - (temp_sevensegment * 10));
            }

            if (junc2_remaining_emergency_green_time[0] < 10)
            {
                sevenSegment9.Value = Convert.ToString(0);
                sevenSegment10.Value = Convert.ToString(junc2_remaining_emergency_green_time[0]);
            }

            else if (junc2_remaining_emergency_green_time[0] >= 10)
            {
                temp_sevensegment = junc2_remaining_emergency_green_time[0] / 10;
                sevenSegment9.Value = Convert.ToString(junc2_remaining_emergency_green_time[0] / 10);
                sevenSegment10.Value = Convert.ToString(junc2_remaining_emergency_green_time[0] - (temp_sevensegment * 10));
            }

            if (junc2_remaining_emergency_green_time[1] < 10)
            {
                sevenSegment11.Value = Convert.ToString(0);
                sevenSegment12.Value = Convert.ToString(junc2_remaining_emergency_green_time[1]);
            }

            else if (junc2_remaining_emergency_green_time[1] >= 10)
            {
                temp_sevensegment = junc2_remaining_emergency_green_time[1] / 10;
                sevenSegment11.Value = Convert.ToString(junc2_remaining_emergency_green_time[1] / 10);
                sevenSegment12.Value = Convert.ToString(junc2_remaining_emergency_green_time[1] - (temp_sevensegment * 10));
            }

            if (junc2_remaining_emergency_green_time[2] < 10)
            {
                sevenSegment13.Value = Convert.ToString(0);
                sevenSegment14.Value = Convert.ToString(junc2_remaining_emergency_green_time[2]);
            }

            else if (junc2_remaining_emergency_green_time[2] >= 10)
            {
                temp_sevensegment = junc2_remaining_emergency_green_time[2] / 10;
                sevenSegment13.Value = Convert.ToString(junc2_remaining_emergency_green_time[2] / 10);
                sevenSegment14.Value = Convert.ToString(junc2_remaining_emergency_green_time[2] - (temp_sevensegment * 10));
            }

            if (junc2_remaining_emergency_green_time[3] < 10)
            {
                sevenSegment15.Value = Convert.ToString(0);
                sevenSegment16.Value = Convert.ToString(junc2_remaining_emergency_green_time[3]);
            }

            else if (junc2_remaining_emergency_green_time[3] >= 10)
            {
                temp_sevensegment = junc2_remaining_emergency_green_time[3] / 10;
                sevenSegment15.Value = Convert.ToString(junc2_remaining_emergency_green_time[3] / 10);
                sevenSegment16.Value = Convert.ToString(junc2_remaining_emergency_green_time[3] - (temp_sevensegment * 10));
            }
        }
        ...
        private async static void Junction1_SendCloudToDeviceMessageAsync(Junction junction)
        {
            ...
            // Send Data
            JSONEdison temp_junction_data = new JSONEdison();
            temp_junction_data.Name = "SetGreenTime";
            temp_junction_data.Parameters.north_green_time = junction.north_green_time;
            temp_junction_data.Parameters.north_priority = junction.north_priority;
            temp_junction_data.Parameters.east_green_time = junction.east_green_time;
            temp_junction_data.Parameters.east_priority = junction.east_priority;
            temp_junction_data.Parameters.south_green_time = junction.south_green_time;
            temp_junction_data.Parameters.south_priority = junction.south_priority;
            temp_junction_data.Parameters.west_green_time = junction.west_green_time;
            temp_junction_data.Parameters.west_priority = junction.west_priority;

            var messageString3 = JsonConvert.SerializeObject(temp_junction_data, new Newtonsoft.Json.Converters.KeyValuePairConverter());
            var edison1_data = new Microsoft.Azure.Devices.Message(Encoding.ASCII.GetBytes(messageString3));
            edison1_data.Ack = DeliveryAcknowledgement.Full;
            await serviceClient.SendAsync("Edison01", edison1_data);

            trial_send++;
        }

        private async static void Junction2_SendCloudToDeviceMessageAsync(Junction junction)
        {
            // Send Data
            JSONEdison temp_junction_data = new JSONEdison();
            temp_junction_data.Name = "SetGreenTime";
            ...
            var messageString3 = JsonConvert.SerializeObject(temp_junction_data, new Newtonsoft.Json.Converters.KeyValuePairConverter());
            var edison2_data = new Microsoft.Azure.Devices.Message(Encoding.ASCII.GetBytes(messageString3));
            edison2_data.Ack = DeliveryAcknowledgement.Full;
            await serviceClient.SendAsync("Edison02", edison2_data);
