...
// Global Function
//extern EXECUTE_COMMAND_RESULT SetGreenTime(ContosoAnemometer* device, int north_green_time, int north_priority, int east_green_time, int east_priority, int south_green_time, int south_priority, int west_green_time, int west_priority);
//extern void sendCallback(IOTHUB_CLIENT_CONFIRMATION_RESULT result, void* userContextCallback);
//extern static void sendMessage(IOTHUB_CLIENT_HANDLE iotHubClientHandle, const unsigned char* buffer, size_t size);
//extern static IOTHUBMESSAGE_DISPOSITION_RESULT IoTHubMessage(IOTHUB_MESSAGE_HANDLE message, void* userContextCallback);
